...
class usbSerial(QtWidgets.QWidget): #Implementation based on: https://stackoverflow.com/questions/55070483/connect-to-serial-from-a-pyqt-gui
    def __init__(self, gui, parent=None):
        ...
        self.default_action_number = len(self.gui.menu_connection_controllers.actions()) #Get number of actions in connection menu when there are no connected drivers
        ...
        self.conn_menu_action_group = QtWidgets.QActionGroup(self.gui.menu_connection_controllers) #Connection menu action group to have options act like radio buttons
    ...
    def getDriverPort(self, on_boot = False):
        ...
        if on_boot: #On boot, automatically connect to the first driver in the menu
            ...
            if len(self.gui.menu_connection_controllers.actions()) > self.default_action_number:
                self.showMessage("Success: " + str(len(self.gui.menu_connection_controllers.actions()) - self.default_action_number) + " LED driver(s) were found.")
            else:
    ...
    def connectSerial(self, port):
        ...
        #except: #Return False if unable to establish connection to serial port
            if debug:
                print("Failed to connect to COM port: " + str(port) + ", with QSerialPort Error #" + str(self.active_port.error()))
            self.disconnectSerial()
            return False
    ...
    def onTriggered(self, action):
        if str(action.objectName()) in ["menu_connection_disconnect", "menu_connection_controllers_disconnect"]:
            ...
            if self.connectSerial(port):
                ...
                self.showMessage("Error: Failed to open controller port.  Check USB connection and confirm no other software is connected to the driver.")
    ...
    def downloadDriverId(self, reply=None):
        if reply is not None:
            ...
            for action in self.gui.menu_connection_controllers.actions():
                ...
                self.gui.menu_connection_controllers.insertAction(self.gui.menu_connection_controllers_disconnect, menu_item)
