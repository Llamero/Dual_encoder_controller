...
class usbSerial(QtWidgets.QWidget): #Implementation based on: https://stackoverflow.com/questions/55070483/connect-to-serial-from-a-pyqt-gui
    def __init__(self, gui, parent=None):
        ...
        self.conn_menu_action_group = QtWidgets.QActionGroup(self.gui.menu_connection_drivers) #Connection menu action group to have options act like radio buttons
        ...
        for action in self.gui.menu_connection_drivers.actions():
    ...
    def getDriverPort(self, on_boot = False):
        ...
        if on_boot: #On boot, automatically connect to the first driver in the menu
            action = self.gui.menu_connection_drivers.actions()[0]
            action.setChecked(True)
            self.onTriggered(action)
        else: #If check was performed through menu, inform of result
            if len(self.gui.menu_connection_drivers.actions()) > self.default_action_number:
                self.showMessage("Success: " + str(len(self.gui.menu_connection_drivers.actions()) - self.default_action_number) + " LED driver(s) were found.")
            else:
    ...
    def disconnectSerial(self, error = None):
        if error in [None, QSerialPort.SerialPortError.ResourceError, QSerialPort.SerialPortError.DeviceNotFoundError]:
            ...
            self.gui.menu_connection_drivers_disconnect.setChecked(True)
    ...
    def onTriggered(self, action):
        if str(action.objectName()) in ["menu_connection_disconnect", "menu_connection_drivers_disconnect"]:
            self.disconnectSerial()
            ...
            if self.connectSerial(port):
                ...
                self.gui.menu_connection_drivers.removeAction(action)
    ...
    def downloadDriverId(self, reply=None):
        if reply is not None:
            ...
            for action in self.gui.menu_connection_drivers.actions():
                ...
                self.gui.menu_connection_drivers.insertAction(self.gui.menu_connection_drivers_disconnect, menu_item)
